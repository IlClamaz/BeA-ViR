...
{
    [SerializeField] CaveWall caveWall;
    ...
    {
        get { return caveWall.transform; }
    }

    public CaveWall Wall
    {
        set
        {
            caveWall = value;

            if (caveWall.transform)
            {
                _isDirty = true;
            }
            else
    ...
    void OnEnable()
    {
        ...
        {
            if (caveWall.ShouldBeActive == false)
            {
                _cam.enabled = false;
    ...
    void LateUpdate()
    {
        ...
        _winCorners[0] = caveWall.transform.TransformPoint(-0.5f, -0.5f, 0);
        _winCorners[1] = caveWall.transform.TransformPoint(-0.5f, 0.5f, 0);
        _winCorners[2] = caveWall.transform.TransformPoint(0.5f, 0.5f, 0);
        _winCorners[3] = caveWall.transform.TransformPoint(0.5f, -0.5f, 0);


        // Compute and apply off-axis perspective.
        if (caveWall.transform.hasChanged || transform.hasChanged || _isDirty)
        {
            ...
            {
                Vector3 camForward = caveWall.transform.position - transform.position;
                float windowDist = camForward.magnitude;
                camForward /= windowDist; // Normalize

                Vector3 camUp = Vector3.Cross(camForward, caveWall.transform.right).normalized;
                Vector3 camRight = Vector3.Cross(caveWall.transform.up, camForward).normalized;
            ...
            caveWall.transform.hasChanged = false;
